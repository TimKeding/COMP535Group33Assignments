...
void gncCmd() {
    ...
    // -u for UDP
        ...
        udp_recv_fn recv_callback = udp_recv_callback;
        err_t (*send_fn)(struct udp_pcb *, struct pbuf *) = udp_send;
        int rdp_mode = 0;

        // RDP
        if (!strcmp(next_tok, "-r")) {
            int window_size=1;
            long timeout=1000;
            ...
            if(!strcmp(next_tok, "-t")) {
                next_tok = strtok(NULL, " \n");
                timeout = atol(next_tok);

                next_tok = strtok(NULL, " \n");
            }

            rdp_init(window_size, timeout);
        }
        ...
        if (!strcmp(next_tok, "-l")) {
            ...
            udp_recv(pcb, recv_callback, pcb);
            uchar any[4] = {0,0,0,0};
